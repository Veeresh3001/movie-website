...
class Home extends Component {
  state = {
    trending: [],
    original: [],
    treindingApiStatus: treindingApiCon.initial,
    originalApiStatus: originalApiCon.initial,
  }
  ...
  getTrendingData = async () => {
    this.setState({
      treindingApiStatus: treindingApiCon.loading,
    })
    ...
    if (trendingRes.ok === true) {
      ...
      this.setState({
        trending: updateRes,
        treindingApiStatus: treindingApiCon.succuss,
      })
    } else {
      this.setState({treindingApiStatus: treindingApiCon.failure})
    }
  }

  getOriginalData = async () => {
    this.setState({
      originalApiStatus: originalApiCon.loading,
    })
    const jwt = Cookies.get('jwt_token')
    // console.log(jwt)
    const opt = {
    ...
    if (originalRes.ok === true) {
      ...
      this.setState({
        original: updateRes,
        originalApiStatus: originalApiCon.succuss,
      })
    } else {
      this.setState({originalApiStatus: originalApiCon.failure})
    }
  }
  ...
  loadingOriginalComp = () => (
    <div>
      <Header page="home" />
      <div className="loader-container banner-loading" testid="loader">
        <Loader type="TailSpin" color="#D81F26" height={50} width={50} />
      </div>
    </div>
  )
  ...
  render() {
    const {original, trending, treindingApiStatus, originalApiStatus} =
      this.state
    return (
      <div className="home">
        {this.orignalRenderSwitch(original, originalApiStatus)}
        <div className="home-card">
          <h1 className="home-head">Trending Now</h1>
          {this.treindingRenderSwitch(trending, treindingApiStatus)}
          <h1 className="home-head">Originals</h1>
          {this.treindingRenderSwitch(original, originalApiStatus)}
        </div>
